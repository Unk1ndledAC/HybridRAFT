    ...
    def vali(self, vali_data, vali_loader, criterion, mode='valid'):
        ...
        with torch.no_grad():
            for i, (index, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                # encoder - decoder
                if self.args.model == 'HybridRAFT':
                    outputs = self.model(batch_x, index, batch_x_mark, mode=mode)
...
#         early_stopping = EarlyStopping(patience=self.args.patience, verbose=True)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (index, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # encoder - decoder
                if self.args.model == 'HybridRAFT':
                    outputs = self.model(batch_x, index, batch_x_mark, mode='train')
                elif self.args.model == 'RAFT':
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (index, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # encoder - decoder
                if self.args.model == 'HybridRAFT':
                    outputs = self.model(batch_x, index, batch_x_mark, mode='test')
                elif self.args.model == 'RAFT':
